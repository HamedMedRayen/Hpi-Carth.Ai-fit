...
The `Hpi` repository is a complete, production-grade full-stack cross-platform application (Web & Android) designed for comprehensive workout tracking, physiological recovery analysis, smart nutrition logging, and personalized coaching.
...
- **Frontend (React)**: A visually dense, glassmorphism-themed UI utilizing `Recharts` for interactive analytics, featuring rich modules like interactive Borg RPE fatigue tracking, calorie logging with smart AI scanning, body sizing maps, visual progress comparison, and fitness challenges.
...
Hpi relies on **PostgreSQL** for relational storage and **Groq** for conversational AI execution:
...
3. Create a free Groq API Key to enable the Hpi floating assistant and smart nutrition text scanning.
...
# Install dependencies (automatically installs groq, pandas, fastapi, etc.)
...
2. Built all necessary SQL schema tables (`users`, `workouts`, `sets`, `measurements`, `challenges`, `nutrition_logs`, etc.) with proper constraints.
...
2. **Open Android Studio:**
   ...
   ```
3. **Run on Device or Emulator:** Use the "Run" button in Android Studio to deploy the app to your physical phone via USB debugging, or to a virtual device via the Device Manager.

---

## Core Workflows to Explore

### 1. Hpi AI Fitness Assistant
...
### 2. Smart Nutrition Hub & AI Scanner
Navigate to the `Nutrition Hub` tab:
- **AI Scanning**: Type what you ate in natural language (e.g. *"three eggs, a slice of avocado toast, and a protein shake"*) and submit. The Groq parser automatically calculates the calorie and macronutrient breakdown and saves it to your daily log.
...
### 5. Growth Trends & Sizing
Navigate to `Measurements`:
- Input your body metrics (Waist, Arm, Chest, etc.).
- The visual anatomical map displays comparative flags (Green = Decreased, Red = Increased, Cyan = Stable) based on your preceding session, charting progress trends over time.
...
**`groq` Package Missing Errors**
→ Make sure you have activated your virtual environment before running the server, or execute `pip install groq` manually.
